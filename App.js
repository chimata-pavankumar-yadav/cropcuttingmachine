...
function CustomHeader({ navigation, header }) {
  ...
  useEffect(() => {
    db.transaction(tx => {
      ...
      tx.executeSql(
        "create table if not exists wheeldatabase (id integer primary key not null, twowheel int, fourwheel int);"
      );
      tx.executeSql(
        `select * from wheeldatabase where id = ? ;`,
        [1],
        (_, { rows: { _array } }) =>
          setTwowheeldetails(_array[0].twowheel)
      );
      tx.executeSql(
        `select * from wheeldatabase where id = ? ;`,
        [1],
        (_, { rows: { _array } }) =>
          setFourwheeldetails(_array[0].fourwheel)
      );
    });
  }, []);
  ...
  const wheeldata = (data) => {
    ...
    db.transaction(
      tx => {
        ...
        if (twowheeldetails === null && fourwheeldetails === null) {
          tx.executeSql("insert into wheeldatabase (twowheel,fourwheel) values (?, ?)",
            [data.twowheel, data.fourwheel]);
          tx.executeSql(
            `select * from wheeldatabase where id = ? ;`,
            [1],
            (_, { rows: { _array } }) =>
          ...
          tx.executeSql('UPDATE wheeldatabase set twowheel=?, fourwheel=?  where id=?',
  ...
  const errorhandler = yup.object({
    ...
    twowheel: yup.string().required().min(1),
    fourwheel: yup.string().required().min(1),
  })
    ...
    <View style={{ flexDirection: 'row', height: 60, borderWidth: 1, borderColor: '#101010' }}>
      ...
      <View style={{ flex: 1, marginLeft: 7, justifyContent: 'center' }}>
        ...
        <Text style={{ color: 'yellowgreen' }}>
          2 <Image source={tyre} style={{ width: 19, height: 19 }} /> /minute  $<Text style={{ color: 'white' }}>{twowheeldetails}</Text>
        </Text>
        <Text style={{ color: 'yellowgreen' }}>
          4 <Image source={tyre} style={{ width: 19, height: 19 }} /> /minute  $<Text style={{ color: 'white' }}>{fourwheeldetails}</Text>
        </Text>
      ...
      <TouchableOpacity onPress={() => setModelwheel(true)} style={{ flex: 1, justifyContent: 'center', alignItems: 'flex-end', marginRight: 10 }}>
        <Ionicons name="md-settings" size={24} color="yellowgreen" />
      </TouchableOpacity>


      <Modal visible={modelwheel} transparent={true} animationType={'slide'}>
        <BlurView intensity={200} tint='dark' style={[StyleSheet.absoluteFill, { height: 300, marginTop: 150, marginBottom: 200, marginLeft: 20, marginRight: 20, borderRadius: 10 }]}>
          <View style={{ flexDirection: 'row' }}>
            ...
            <View style={{ flex: .5 }}>
              <TouchableHighlight onPress={() => setModelwheel(!modelwheel)} style={{ alignItems: 'flex-end' }}>
...
function Tabletab({ tabletabname, personname, onPressItem, twoprice, fourprice, navigation }) {
  ...
  useEffect(() => {
    db.transaction(tx => {
      ...
      tx.executeSql(
        `select * from wheeldatabase where id = 1;`, [],
          ...
          renderItem={({ item }) => (
            <View style={{ flexDirection: 'row', }}>
              ...
              <View style={{ flex: 1, flexDirection: 'row' }}>
                ...
                <View style={{ flex: 2 }}>
                  {((item.wheeler) === '2') ? <Text style={{ padding: 8, color: 'white', fontSize: 15, }}>{item.hour * (twowheelprice * 60) + item.minutes * twowheelprice}</Text> : <Text style={{ padding: 8, color: 'white', fontSize: 15 }}>{item.hour * (fourwheelprice * 60) + item.minutes * fourwheelprice}</Text>}
                </View>
